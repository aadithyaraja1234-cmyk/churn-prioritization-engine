...
- `models/v1/`, `models/banking_v1/` - Telco/Banking's versioned, committed model artifacts. Three self-registered tenants are ALSO permanently committed as reference fixtures - `models/aurora-streaming/`, `models/meridian-wireless/`, `models/fernwood-retail-collective/` (with their filtered training CSVs at the matching paths under `data/tenant_uploads/`) - real regression coverage for genuinely different column/id-column names and partial-module-coverage cases that multiple tests depend on directly (see `.gitignore`'s own comment for the full reasoning). Every OTHER self-registered tenant's artifacts land under `models/{tenant_id}/`, gitignored (regenerated by training, not source of truth).
